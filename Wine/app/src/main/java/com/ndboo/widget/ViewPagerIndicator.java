...
public class ViewPagerIndicator extends LinearLayout {
    ...
    public void scroll(int position, float offset) {
        ...
        // 容器滚动，当移动到倒数最后mStopPosition个的时候，开始滚动
        //当position+mStopPosition=size时不滚动
        if (offset > 0 && position >= (mTabVisibleCount - mStopPosition)
                && getChildCount() > mTabVisibleCount
                && position != mTabTitles.length - mStopPosition) {
    ...
    public int getScreenWidth() {
        return getResources().getDisplayMetrics().widthPixels - getPaddingRight() - getPaddingLeft();
    }
}